...
func setOutput(logger *logrus.Logger, config conf.LogConfig) {
...
func (c *CustomizedFormatter) Format(entry *logrus.Entry) ([]byte, error) {
	detailInfo := ""
	if entry.Caller != nil {
		function := strings.ReplaceAll(entry.Caller.Function, "github.com/FixedShadow/jammy-cloud-database/rds-api", "")
